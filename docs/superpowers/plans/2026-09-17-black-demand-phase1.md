...
- Produces:
  ...
  - `uber_import.MAX_MEMBER_BYTES = 150 * 1024 * 1024`; a larger `analytics` member is skipped and listed in `files_missing` once, with consequence "driver_app_analytics.csv is larger than 150 MB; not read." (kind `analytics_too_large`; it is NOT also listed as `analytics` missing). Analytics rows that fail to parse count in `ParsedExport.skipped_rows` and in `gps.skipped_rows`.
  ...
  - `gps_import.segment_pings(pings, trips, *, home: tuple[float, float] | None, home_radius_m: int) -> list[ParsedSegment]` — pure; the rules of Addendum A "Segmentation" (busy = `[min(request_at, begin_at), dropoff_at]`; no `enroute` when `begin_at` precedes `request_at`; home tag = time-weighted majority of the segment's pings within the radius); states `open`/`enroute`/`ontrip` only; `dedup_key = sha256("gps|state|begin_at.isoformat()|end_at.isoformat()|h3_r8 or ''")`.
  ...
  - `gps_import.top_waits(open_segments, premium_cells, *, places: dict[str, tuple[float, float]], home_cell: str | None) -> list[dict]` — pure; `home_cell` (the H3 res-8 cell of `DEMAND_HOME_LAT/LNG`, or None) is never listed; items exactly `{"h3_r8", "hours", "premium_requests", "per_hour", "place", "distance_km", "zone_key", "zone_name", "outside"}` per Addendum A; `hours` rounded to 1 decimal, `per_hour` = `premium_requests` ÷ unrounded hours, rounded to 2 decimals; `home`-tagged segments excluded by the caller and `home_cell` excluded inside; sorted by minutes desc, `TOP_WAITS` items.
...
7. `test_home_majority_rule` (pure, `segment_pings` only, `home_radius_m=100`): with a point P 250 m east of HOME that lies in HOME's own res-8 cell (assert it with `h3`; move it closer if not), 1 min of pings at P then 20 min at HOME → one open segment tagged `home`; 20 min at P then 1 min at HOME → one open segment with `zone_key is None`.
